...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # CNN: (src_len, b, e) -> (b, e, src_len) -> (src_len, b, e)
        src_len = source_padded.size(0)
        X = X.permute(1, 2, 0).contiguous()  # (b,e,src_len)
        X = self.post_embed_cnn(X)  # (b,e,src_len+1)
        X = X[:, :, 1:src_len+1]  # 裁回 (b,e,src_len)
        X = X.permute(2, 0, 1).contiguous()  # (src_len,b,e)

        #
        X_packed = pack_padded_sequence(input=X,
                                        lengths=source_lengths,
                                        enforce_sorted=True)
        # (S, B, 2H), (L*2, B, H), (L*2, B, H)
        ...
        #
        b = source_padded.size(1)
        # h_n = h_n.view(L_enc, D, B, H)
        h_n = last_hidden.view(self.encoder.num_layers, 2, b, self.hidden_size)
        c_n   = last_cell.view(self.encoder.num_layers, 2, b, self.hidden_size)
        # The last layer of Stacked LSTM
        # h_last -> [D, B, H]
        h_last, c_last = h_n[-1], c_n[-1]
        # hidden state of forward pass and backward pass -> [B, H]
        h_fwd, h_bwd = h_last[0], h_last[1]
        c_fwd, c_bwd = c_last[0], c_last[1]
        # (B, 2H)
        h_cat = torch.cat([h_fwd, h_bwd], dim=1)
        c_cat = torch.cat([c_fwd, c_bwd], dim=1)
        # (B, 2H) * (2H, H_dec) = (B, H_dec)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ### YOUR CODE HERE (~9 Lines)
        # attention projection - (b, src_len, h*2) * (h, 2h) - (b, src_len, h)
        enc_hiddens_proj = self.att_projection(enc_hiddens)
        # Construct Y = (tgt_len, b) -> (tgt_len, b, e)
        Y = self.model_embeddings.target(target_padded)

        #time_chunks = torch.split(Y, 1, dim=0)
        #for t, Y_t in enumerate(time_chunks):
        for Y_t in Y:
            # (1, b, e) - (b, e)
            # Y_t = torch.squeeze(Y_t,dim=0)
            # o_pre - (b, h) Y_t (b, e) = (b, h + e)
            Ybar_t = torch.cat([Y_t, o_prev], dim=1)
            dec_state, o_t, _ = self.step(Ybar_t=Ybar_t, dec_state=dec_state,
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # (b, h) - (b, 1, h)
        ht_3d = torch.unsqueeze(ht, dim=1)
        # (b, 1, h) (b, h, s) - (b, 1, s)
        e_t = torch.matmul(ht_3d, enc_hiddens_proj.transpose(1, 2))
        # (b, s)
        ...
        # (b, 1, s) (b, s, 2h) = (b, 1, 2h) = (b, 2h)
        a_t = torch.unsqueeze(alpha_t, dim=1).matmul(enc_hiddens).squeeze(dim=1)
        # (b, 2h) + (b, h) = (b, 3h)
        u_t = torch.cat([a_t, ht], dim=1)
        # (h, 3h) (b, 3h) - (b, h)
