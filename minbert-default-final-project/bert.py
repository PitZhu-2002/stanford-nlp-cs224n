...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # Q, K, V shape = (B, H, S, D)
    # scores = (B, H, S, D) @ (B, H, D, S) -> (B, H, S, S) /
    b, h, s, d = query.size()
    scores = (query @ key.transpose(-1, -2)) / math.sqrt(d)
    # mask the pad position with -inf
    # (B, H, S, S) + (B, 1, 1, S) -> (B, H, S, S)
    scores = scores + attention_mask
    # (B, H, S, S)
    attn_probs = torch.softmax(scores, dim=-1)
    # scores (B, H, S, S) * value (B, H, S, D) = (B, H, S, D)
    weighted_value = attn_probs @ value
    # (B, H, S, D) -> (B, S, H, D) -> (B, S, H * D)
    output = weighted_value.transpose(1, 2).reshape(b, s, -1)
    return output
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    dense_output = self.interm_dense(output)
    af_output = self.interm_af(dense_output)

    output = self.add_norm(output, af_output,
